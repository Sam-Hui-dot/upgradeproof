...
python3 -c 'import glob,json; r=json.load(open(sorted(glob.glob(".artifacts/*/report.json"))[-1])); assert r["overall_status"] == "passed"; assert r["paths"][0]["checks"][0]["status"] == "passed"'
