...
python3 -c 'import glob,json; r=json.load(open(sorted(glob.glob(".artifacts/*/report.json"))[-1])); assert r["overall_status"] == "passed"; assert len(r["paths"][0]["resolved_images"]) == 3'
